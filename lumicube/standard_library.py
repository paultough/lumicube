"""
This file is taken directly from the lumicube open source repo with a few modifications so that you can run commands using the 
lumicube python API on a remote computer.  This is what I bought my lumicube for - so that I could remotely control it 
based on things happening on my local computer.  I was frustrated that I needed to access the web API on the cube to code it and so this
is my solution which works for my purposes and I supply here for anyone else wanting to do something similar.

*Be nice - I only dust off my Python skills about once per year!*

The original AbstractFoundry code can be found here so you can compare:

https://github.com/abstractfoundry/lumicube-daemon/blob/main/src/main/resources/META-INF/resources/python/foundry_api/standard_library.py
...
The only real changes made are simple and only prevent the default cube from being created as it is expecting to connect
to a host machine with the lumicube deamon installed which my local Mac will not have.  I have added code to throw an exception if
you try to connect to the daemon if it does not exist (ie if you are trying to locally (localhost) connect to the 
lumicube on your remote computer).
...
want to replicate these or just put "cube." in front of your calls to the cube. I prefer the prefixing as I like the 
non-abstracting of the fact the cube API is actually a class. It was a small peev of mine from the manual as it hid something 
important from the coder I think.
"""
...
# Added the following to check if running on a cube and so can connect locally to the deamon
# This is one of the only small changes made to the AbstractFoundry code.
def isRunningOnCube():
    import platform
    
    # Check if the current platform is Raspberry Pi
    if os.path.exists('/tmp/foundry_daemon.sock'):
        return True
    else:
        # Code is not running on a Raspberry Pi (likely on a different platform)
        return False
    ...
    def __init__(self, address=None, port=2020):
        if address is None:
            if isRunningOnCube():
                ...
                raise Exception("Cannot connect to cube.  Specify its IP address or run this code on a cube.")
        else:
...
"""
The following lines have been removed from the vanilla Lumicube source to prevent a default cube from being
created.  The code for creating a default cube will try and connect to '/tmp/foundry_daemon.sock' which
does not exist on a remote computer.  

You can add these to your python code if you prefer this way of connecting to the cube with the right IP address.
